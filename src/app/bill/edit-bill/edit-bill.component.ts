...
export class EditBillComponent implements OnInit {
  ...
  constructor(private datePipe: DatePipe,private http : HttpClient, private route : ActivatedRoute,private fb : FormBuilder ,private router : Router) {
  ...
  fetchBill(e){
    this.http.post('http://localhost/testslim_1/public/fetchBill',{billId : this.billId}).subscribe((response:any) => {
      ...
      if(response.status == true){
        ...
        this.billForm.patchValue({engineersName: this.billDetails.engineersName});
        this.billForm.patchValue({siteLocation: this.billDetails.location});
        this.billForm.patchValue({siteName: this.billDetails.siteName});
        var i = 0;
        response.data.item.forEach(function (row) {
          ...
          e.billForm.addControl("item"+i+"Date",e.fb.control(row.date));
